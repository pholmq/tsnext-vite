...
export const useLevaControls = () => {
  const planetsArray = miscSettings.filter((item) => item.type === "planet").map((item) => item.name);
  ...
  const [values, setControls] = useControls(() => ({
    ...
    Positions: folder({
      tip: { label: "Tip:", value: "You can hover a planet to see its position", editable: false },
    }, { collapsed: true }),
    Camera: folder({
      cameratip: { label: "Tip:", value: "Double click a planet to center the camera on it", editable: false },
      Target: {
    ...
    Planets: folder({ "Planet sizes": { value: 1, min: -5, max: 5, step: 1 } }, { collapsed: true }),
  ...
  const updateControls = useCallback((updates: Partial<any>) => setControlsRef.current(updates), []);
